...
public class MarkdownDoclet extends Standard {
    ...
    public static boolean start(RootDoc root) {
        ...
        File parent = new File("./generated_markdown");
        parent.mkdirs();

        try (
                BufferedWriter matchers = new BufferedWriter(new FileWriter(new File(parent, "1_complete_list_of_matchers.md")));
                BufferedWriter matcherMultiSource = new BufferedWriter(new FileWriter(new File(parent, "2_complete_list_of_matchers_multi_source.md")));
                BufferedWriter filters = new BufferedWriter(new FileWriter(new File(parent, "3_complete_list_of_filters.md")))) {
    ...
    private static void writeHeader(BufferedWriter writer, String title, String permaLink, int navOrder) throws IOException {
        ...
        writer.newLine();
        writer.write("parent: Available Matchers");
        writer.newLine();
        writer.write("nav_order: " + Integer.toString(navOrder));
        writer.newLine();
        writer.write("permalink: " + permaLink);
        writer.newLine();
        writer.write("---");
        writer.newLine();
        writer.write("# " + title);
